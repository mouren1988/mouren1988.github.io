        ...
        setup(p) {
            ...
            (async () => {
                if (ve.id) {
                   ...
                   n.params = t;
                   // 首先请求第一个 URL
                   let l = await e.get("http://138.2.121.84:8000/jzWeb/options/", n);
                   // 检查返回的数据是否为空
                   if (!l || l.length === 0) {
                       // 如果为空，则请求第二个 URL
                       l = await e.get("http://138.2.121.84:8000/jzWeb/options/", n);
                   }
                   // 处理返回的数据
                   l = l.map((e) => {
                      const t = {};
            ...
            const Ee = async () => {
                ...
                const n = {};
                n.params = t;
                let l = await e.get("http://138.2.121.84:8000/jzWeb/bill/", n);
                if (!l || l.length === 0) {
                    l = await e.get("http://138.2.121.84:8000/jzWeb/bill/", n);
                }
                // console.log(l);
