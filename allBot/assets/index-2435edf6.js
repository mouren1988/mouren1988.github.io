    ...
    Ee = p(d({
        ...
        setup(p) {
            ...
            (async () => {
                if (ve.id) {
                   ...
                   n.params = t;
                   // 首先请求第一个 URL
                   let l = await e.get("http://138.2.121.84:8000/botWeb/options/", n);
                   // 检查返回的数据是否为空
                   if (!l || l.length === 0) {
                       // 如果为空，则请求第二个 URL
                       l = await e.get("http://138.2.121.84:8000/botWeb/options/", n);
                   }
                   // 处理返回的数据
                   l = l.map((e) => {
                      const t = {};
            ...
            const Ee = async () => {
                ...
                const n = {};
                n.params = t;
                let l = await e.get("http://138.2.121.84:8000/botWeb/bill/", n);
                if (!l || l.length === 0) {
                    l = await e.get("http://138.2.121.84:8000/botWeb/bill/", n);
                }
                // console.log(l);
                localStorage.setItem('result', JSON.stringify(l));
                if (l.msg) alert(l.msg);
                if (l.xxx) document.cookie = "token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
                if (l.xxx) location.reload();
     ...
     // 为选择框添加 change 事件监听器
        document.getElementById('fruits').addEventListener('change', function() {
            var okselectedValue = this.value; // 获取选中的值
            if (okselectedValue) { // 如果选中的是有效选项
                okselecvalue = okselectedValue; // 弹窗提示选中的值和文本
            }
        });
// 获取所有具有特定类名的元素
